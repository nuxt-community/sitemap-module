...
const { createRoutesCache, createDynamicRoutesCache, getDynamicRoutes, joinRoutes } = require('./cache.js')
...
function registerSitemap(options, globalCache, nuxtInstance) {
  ...
  // Init dynamic routes cache
  const dynamicRoutesCache = createDynamicRoutesCache(options)
  ...
  if (options.gzip) {
    ...
    nuxtInstance.addServerMiddleware({
      ...
      async handler(req, res, next) {
        try {
          // Init sitemap
          const staticRoutes = await cache.routes.get('routes')
          const dynamicRoutes = await getDynamicRoutes(options, dynamicRoutesCache, req)
          const routes = joinRoutes(staticRoutes, dynamicRoutes)
